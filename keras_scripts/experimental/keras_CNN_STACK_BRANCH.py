...
def build_CNN_brl(X_train,Y_train,conv_pool_n,filter_n,droput_rates,batch_sizes):
    ...
    tf.keras.utils.plot_model(model_cnn, to_file='model_cnn_stack.png', show_shapes=True)
    return(model_cnn)
...
def main():
    ...
    #Regression BL
    #Run model
    model_cnn_reg=build_CNN_brl(X_train=X_train,Y_train=Y_train,conv_pool_n=6,filter_n=1000,droput_rates=0.15,batch_sizes=32)
    ...
    if args.TRANS == "log":
        np.savetxt("brls.evaluated.cnnstack.log.txt",evals_reg,fmt='%f')
        np.savetxt("brls.predicted.cnnstack.log.txt",np.exp(bls),fmt='%f')
        np.savetxt("brls.predicted_train.cnnstack.log.txt",np.exp(train_bls),fmt='%f')
        
    elif args.TRANS == "sqrt":
        np.savetxt("brls.evaluated.cnnstack.sqrt.txt",evals_reg,fmt='%f')
        np.savetxt("brls.predicted.cnnstack.sqrt.txt",np.power(bls,2),fmt='%f')
        np.savetxt("brls.predicted_train.cnnstack.sqrt.txt",np.power(train_bls,2),fmt='%f')
    else:
        np.savetxt("brls.evaluated.cnnstack.txt",evals_reg,fmt='%f')
        np.savetxt("brls.predicted.cnnstack.txt",bls,fmt='%f')
        np.savetxt("brls.predicted_train.cnnstack.txt",train_bls,fmt='%f')
        
    #Saving model
    print("\nSaving keras trained model")
    model_cnn_reg.save("model_cnnstack.h5")
