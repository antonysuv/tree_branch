...
def build_CNN_brl(X_train,Y_train,conv_pool_n,filter_n,droput_rates,batch_sizes):
    ...
    for l in list(range(0,conv_pool_n)):
        x1 = ZeroPadding2D(padding=((0, 0), (0,conv_y[l]-1)))(x1)        
        x1 = Conv2D(filters=filter_s[l], kernel_size=(conv_x[l], conv_y[l]), strides=1,activation='relu')(x1)
        x1 = Dropout(rate=droput_rates)(x1)
        x1 = AveragePooling2D(pool_size=(1,pool[l]))(x1)
        x1 = Dropout(rate=droput_rates)(x1)
    output_msa1 = Flatten()(x1) 
    hidden1 = Dense(1000,activation='relu')(output_msa1)
    drop1 = Dropout(rate=droput_rates)(hidden1)
    
    x2 = visible_msa
    for l in list(range(0,conv_pool_n)):
        x2 = ZeroPadding2D(padding=((0, 0), (0,conv_y[l]-1)))(x2)        
        x2 = Conv2D(filters=filter_s[l], kernel_size=(conv_x[l], conv_y[l]), strides=1,activation='relu')(x2)
        x2 = Dropout(rate=droput_rates)(x2)
        x2 = AveragePooling2D(pool_size=(1,pool[l]))(x2)
        x2 = Dropout(rate=droput_rates)(x2)
    output_msa2 = Flatten()(x2)
    hidden2 = Dense(1000,activation='relu')(output_msa2)
    drop2 = Dropout(rate=droput_rates)(hidden2)
    
    x3 = visible_msa
    for l in list(range(0,conv_pool_n)):
        x3 = ZeroPadding2D(padding=((0, 0), (0,conv_y[l]-1)))(x3)        
        x3 = Conv2D(filters=filter_s[l], kernel_size=(conv_x[l], conv_y[l]), strides=1,activation='relu')(x3)
        x3 = Dropout(rate=droput_rates)(x3)
        x3 = AveragePooling2D(pool_size=(1,pool[l]))(x3)
        x3 = Dropout(rate=droput_rates)(x3)
    output_msa3 = Flatten()(x3)
    hidden3 = Dense(1000,activation='relu')(output_msa3)
    drop3 = Dropout(rate=droput_rates)(hidden3)
      
    merge = concatenate([drop1,drop2,drop3])
    
    output = Dense(N_branch, activation='linear')(merge)
